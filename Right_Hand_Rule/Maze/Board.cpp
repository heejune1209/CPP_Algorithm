...
// 프림 알고리즘은 매 순간 최선의 조건을 선택하는 그리디 알고리즘을 바탕에 둔다. 
...
// 프림 알고리즘의 순서
// 1. 시작 정점을 선택하고, 이를 MST에 포함시킨다.
// 2. MST에 포함된 정점과 포함되지 않은 정점 사이의 가장 작은 가중치를 가진 간선을 선택한다.
// 3. 선택된 간선의 정점을 MST에 추가한다.
// 4. 모든 정점이 MST에 포함될 때까지 위 과정을 반복.

// 프림 알고리즘 시간 복잡도
// 모든 노드에 대해 탐색을 진행하므로 O(V)이다.(v는 정점의 개수)
// 그리고 우선순위 큐를 사용하여 매 노드마다 최소 간선을 찾는 시간은 O(logV)이다. 따라서 탐색과정엔 O(VlogV)가 소요

// 가지를 모아서 둥지를 짓는 느낌

// 크루스칼 vs 프림 알고리즘, Sparse Graph vs Dense Graph
// 간선의 개수가 정점 수에 비해 매우 적은 그래프인 Sparse Graph (희소 그래프)
// 희소 그래프는 일반적으로 간선 수 O(V)에 가까운 경우를 말한다.
// Dense Graph(밀집 그래프) : 간선의 개수가 정점 수에 비해 많은 그래프이다.
// 일반적으로 간선 수 O(V^2)에 가까운 경우를 말한다.

// Sparse Graph에선 정렬에 드는 시간이 적고, 
// 간선 수가 적기 때문에 union-find 연산도 빠르기 때문에 크루스칼 알고리즘이 유리하다.

// Dense Graph는 간선수가 많더라도 우선순위 큐로 효율적으로 처리가 가능하기 때문에 프림 알고리즘이 유리하다
